...
try {
    ...
    // Obtener la fecha de inicio desde repositorio_distributivo
    $queryFechaInicio = "
         SELECT MIN(fecha_asignacion) AS fecha_inicio
            FROM repositorio_distributivo
            WHERE observaciones = 'Adicional';
    ";
    $fechaInicio = $database->select($queryFechaInicio)[0]['fecha_inicio'];

    // Calcular el fin de mes de la fecha de inicio
    $queryFinMes = "
        SELECT LAST_DAY('$fechaInicio') AS fin_mes
    ";
    $finMes = $database->select($queryFinMes)[0]['fin_mes'];
    ...
    $query = "
        ...
        WHERE STR_TO_DATE(ins.fecha, '%d/%m/%Y') BETWEEN '$fechaInicio' AND '$finMes'

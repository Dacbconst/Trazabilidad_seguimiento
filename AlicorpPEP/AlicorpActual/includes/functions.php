...
function login($usuario, $mysqli) {
  //  if ($stmt = $mysqli->prepare("SELECT supervisor FROM repositorio_locales_supervisores_cliente WHERE supervisor = ? LIMIT 1")) {
      if ($stmt = $mysqli->prepare("(SELECT supervisor FROM repositorio_locales_supervisores_cliente WHERE supervisor = ?)
    UNION
    (SELECT usuario FROM repositorio_cliente_usuarios_universales WHERE usuario = ?)
    LIMIT 1;")) {

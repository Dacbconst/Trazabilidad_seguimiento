              ...
              //  console.log(data);
                ...
                //    console.log(perfectStore);
                    ...
                    for (const categoria in groupedByPerfectStore[perfectStore]) {
                        html += `   <div class="card mb-3  text-center" style="box-shadow: 0 4px 50px rgba(231, 90, 90, 0.2);">
                                    <div  class="card-header fw-bold text-uppercase">${categoria}</div>
                                    <div class="card-body">`;

                        groupedByPerfectStore[perfectStore][categoria].forEach(preg => {
                            html += `<p class="mb-2 w-100 fw-bold"><strong id="pregunta-${preg.id}">${preg.pregunta1}</strong></p>
                                    <p class="text-muted">${preg.variante}</p>
                                    <div class="d-flex align-items-center gap-4 mb-3">`;

                            // Verificar si la categoría contiene 'precios'
    ...
    <div class="form-group w-50">
        ...
        <span id="validacion-${preg.id}" class="badge d-none mt-2"></span>
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="respuesta_${preg.id}" id="si_${preg.id}"  onchange="habilitarPrecioInput('${preg.pregunta1}', true)"  ${preg.puntaje == 0 ? 'disabled' : 'required'}>
                        <label class="form-check-label" for="si_${preg.id}">Sí cumple</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="respuesta_${preg.id}" id="no_${preg.id}" onchange="habilitarPrecioInput('${preg.pregunta1}', false)" ${preg.puntaje == 0 ? 'checked disabled' : ''}>
                    ...
                    <div class="dropdown text-center">
                        <button class="btn btn-secondary1 dropdown-toggle" type="button" data-bs-toggle="dropdown" 
                            id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" ${preg.puntaje == 0 ? 'disabled' : ''}>
                            Elija Archivo
                        </button>
                        <ol class="dropdown-menu text-center">
                            ...
                            <li>
                                <label class="dropdown-item" for="inputArchivo_${preg.id}" style="cursor: pointer;">
                                    ...
                                    class="d-none" onchange="previsualizarImagen(${preg.id})" accept=".jpg, .jpeg, .png, image/jpeg, image/png, image/jfif"/>
                            </li>
                        </ol>
                    </div>
                </div>
...
// Implementacion previa de novedades, reemplazada por abrirModalNovedades() debajo
...
//    console.log(tipo);
    ...
    if (tipo === "MAYORISTAS" || tipo === "AASSRR") {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateMargin(inputElement) {
        ...
        const pvc = parseFloat(inputElement.dataset.pvc);
        // Asegúrate de que el margen se parsea correctamente (ej. "75%" a 0.75)
        const marginPercentage = parseFloat(inputElement.dataset.margen) / 100; 

        const validationMessageElement = inputElement.closest('.list-group-item').querySelector('.validation-message');
        ...
        if (isNaN(inputValue) || inputValue === 0) {
            // Si el campo está vacío o es 0, no mostrar validación de margen aún.
            // Puedes decidir si quieres un comportamiento diferente aquí, por ejemplo, marcarlo como requerido.
            return;
        }

        if (isNaN(pvc) || isNaN(marginPercentage)) {
            validationMessageElement.textContent = 'Datos de PVC o Margen no válidos.';
        ...
        const lowerBound = pvc * (1 - marginPercentage); // Precio mínimo permitido (PVC - Margen)
        const upperBound = pvc * (1 + marginPercentage); // Precio máximo permitido (PVC + Margen)

        if (inputValue < lowerBound) {
            validationMessageElement.textContent = `¡Muy bajo! El precio debe ser al menos $${lowerBound.toFixed(2)}.`;
            validationMessageElement.classList.remove('d-none');
            inputElement.classList.add('is-invalid');
            validationIconElement.innerHTML = '<i class="bi bi-arrow-down-circle-fill text-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="Precio muy bajo"></i>';
            validationIconElement.classList.remove('d-none');
        } else if (inputValue > upperBound) {
            validationMessageElement.textContent = `¡Muy alto! El precio no debe exceder $${upperBound.toFixed(2)}.`;
            validationMessageElement.classList.remove('d-none');
            inputElement.classList.add('is-invalid');
            validationIconElement.innerHTML = '<i class="bi bi-arrow-up-circle-fill text-danger" data-bs-toggle="tooltip" data-bs-placement="top" title="Precio muy alto"></i>';
            validationIconElement.classList.remove('d-none');
        } else {
            validationMessageElement.textContent = 'Precio dentro del margen.';
            validationMessageElement.classList.add('d-none'); // O puedes mantenerlo visible con un mensaje de éxito
            inputElement.classList.add('is-valid');
            validationIconElement.innerHTML = '<i class="bi bi-check-circle-fill text-success" data-bs-toggle="tooltip" data-bs-placement="top" title="Precio correcto"></i>';
            validationIconElement.classList.remove('d-none');
        }
    ...
    function renderPreguntas(preguntas, posId) {
                                    ...
                                    html += `
                                        <div class="list-group-item d-flex flex-column flex-md-row justify-content-between align-items-md-center py-2 px-3">
                                            <div class="me-md-3 mb-2 mb-md-0">
                                                <p class="mb-0 text-dark">${item.sku_description || item.sku}</p>
                                            </div>
                                            <div class="d-flex align-items-center flex-grow-1">
                                                ...
                                                >
                                                <span class="ms-2 validation-icon d-none">
                                                </span>
                                            </div>
                                            <input type="hidden" name="item_id[${item.id}]" value="${item.id}">
                                            <div class="invalid-feedback d-none validation-message text-center w-100 mt-1">
                                                </div>
                                        </div>
                                    `;
                                });
        ...
        form.querySelectorAll('.input-precio').forEach(input => {
            // Obtener pvc y margen del data-attribute (ya se hace en el HTML)
            ...
            input.addEventListener('input', function() {
                ...
                // Llamar a la función de validación de margen después de normalizar el input
                validateMargin(this);
            });
